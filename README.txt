INVITACIÓN DAVID & ESTRELLA — V13 RAYA ELIMINADA DEL PNG

Para GitHub Pages:
1. Descomprime el ZIP.
2. Sube index.html y todos los PNG al mismo nivel del repositorio.
3. Sustituye los archivos anteriores.

Cambios:
- Eliminadas las líneas/marco decorativo de la portada alrededor de David & Estrella.
- Eliminada la fotografía del traje marrón.
- Ampliada la fotografía de la camiseta gris y mostrada completa para que se vea la mano con el anillo.
- Eliminados los horarios posteriores; solo se muestra la llegada a la finca y ceremonia a las 13:00.
- Actualizado el texto de la sección Regalo.
- Eliminado completamente el apartado Dress Code y sus imágenes.
- Recuperados Ceremonia, Cóctel, Comida y Fiesta; solo Ceremonia muestra la hora 13:00.
- Revisadas las rutas, el JavaScript y la compatibilidad con GitHub Pages.
- Eliminada la raya verde izquierda tanto en la pantalla de apertura como en la portada interior.
- La franja verde ha sido borrada físicamente del archivo floral y ambas pantallas usan la imagen limpia.
- Horarios definitivos: Ceremonia 13:00, Cóctel 14:00, Comida 15:30 y Fiesta 17:00.
- Añadido el texto definitivo del regalo y el IBAN facilitado.
- Añadido “Horarios” al menú.
- Se mantiene la conexión existente con Google Sheets y el correo.
